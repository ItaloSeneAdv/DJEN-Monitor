...
def schedule_exists() -> bool:
    ...
    if system == "Windows":
        try:
            ...
            result = _run_hidden(["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", f"if (Get-ScheduledTask -TaskName '{task}' -ErrorAction SilentlyContinue) {{ exit 0 }} else {{ exit 1 }}"])
    ...
    if system == "Darwin":
        ...
        if not plist.exists():
            return False
        try:
            result = subprocess.run(["launchctl", "print", f"gui/{os.getuid()}/{LAUNCHD_LABEL_MACOS}"], capture_output=True, text=True)
            return result.returncode == 0
        except OSError:
            return plist.exists()
    return False
...
def _build_windows_registration_script(horario: str, command: list[str], result_file: Path) -> str:
    exe, args = command[0], " ".join(command[1:])
    user = _windows_current_user()
    hh, mm = [int(value) for value in horario.split(":")]
    task, exe_q, args_q, user_q, result_q = map(_ps_single_quote, [TASK_NAME_WINDOWS, exe, args, user, str(result_file)])
...
try {{
  ...
  if ([string]$check.Actions[0].Execute -ne '{exe_q}') {{ throw "Executavel registrado difere do esperado" }}
  if ([string]$check.Actions[0].Arguments -ne '{args_q}') {{ throw "Argumentos registrados diferem do esperado" }}
  Set-Content -LiteralPath '{result_q}' -Value "OK" -Encoding UTF8
}} catch {{ Set-Content -LiteralPath '{result_q}' -Value ("ERRO: " + $_.Exception.Message) -Encoding UTF8; exit 1 }}
'''
...
def _build_windows_removal_script(result_file: Path) -> str:
    task, result_q = map(_ps_single_quote, [TASK_NAME_WINDOWS, str(result_file)])
    return f'''$ErrorActionPreference = "Stop"
try {{ Unregister-ScheduledTask -TaskName '{task}' -Confirm:$false -ErrorAction SilentlyContinue; Set-Content -LiteralPath '{result_q}' -Value "OK" -Encoding UTF8 }}
catch {{ Set-Content -LiteralPath '{result_q}' -Value ("ERRO: " + $_.Exception.Message) -Encoding UTF8; exit 1 }}
'''
...
def _run_powershell_direct(script: str) -> subprocess.CompletedProcess[str]:
    ...
    return _run_hidden(["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-EncodedCommand", payload])


def _run_powershell_elevated(script: str) -> None:
    payload = base64.b64encode(script.encode("utf-16-le")).decode("ascii")
    outer = "$p = Start-Process -FilePath 'powershell.exe' -Verb RunAs -Wait -PassThru " + f"-ArgumentList @('-NoProfile','-ExecutionPolicy','Bypass','-EncodedCommand','{payload}'); exit $p.ExitCode"
    result = _run_hidden(["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", outer])
    if result.returncode != 0:
        raise SchedulerError((result.stderr or result.stdout or "A elevação do Windows foi cancelada ou falhou.").strip())
...
def _write_marker(horario: str, system: str) -> None:
    _marker_path().write_text(json.dumps({"horario": horario, "sistema": system}, ensure_ascii=False, indent=2), encoding="utf-8")
