...
def test_schedule_exists_checks_native_scheduler_even_without_marker(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_run_hidden", lambda command: types.SimpleNamespace(returncode=0, stdout="", stderr=""))
...
def test_windows_registration_script_has_reliability_settings(monkeypatch, tmp_path):
    monkeypatch.setenv("USERDOMAIN", "PC"); monkeypatch.setenv("USERNAME", "Usuario")
    script = scheduler._build_windows_registration_script("08:15", [r"C:\Users\Usuario\AppData\Local\DJEN Monitor\bin\DJEN Monitor.exe", "--automatico"], tmp_path / "resultado.txt")
...
def test_macos_schedule_exists_uses_launchctl(tmp_path, monkeypatch):
    plist = tmp_path / "agent.plist"; plist.write_text("x")
    monkeypatch.setattr(scheduler.platform, "system", lambda: "Darwin")
    monkeypatch.setattr(scheduler, "_macos_plist_path", lambda: plist)
    monkeypatch.setattr(scheduler.os, "getuid", lambda: 501)
    monkeypatch.setattr(scheduler.subprocess, "run", lambda *a, **k: types.SimpleNamespace(returncode=0, stdout="", stderr=""))
    assert scheduler.schedule_exists() is True
...
def test_linux_scheduler_is_rejected(monkeypatch):
    ...
    try: scheduler.install_daily_schedule("08:00")
    except scheduler.SchedulerError as exc: assert "Windows e macOS" in str(exc)
    else: raise AssertionError("Deveria recusar agendamento no Linux")
